...
class ShowTouchesController {
    ...
    public func touchEnded(_ touch: UITouch, view: UIView) {
        guard let touchStartDate = touchesStartDateDict[String(format: "%p", touch)],
              let touchImgView = touchImageView(for: touch) else { return }
