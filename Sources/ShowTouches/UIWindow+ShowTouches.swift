...
extension UIWindow {
    ...
    var controller: ShowTouchesController {
        var controller: ShowTouchesController? = objc_getAssociatedObject(
            self,
            &UIWindow.touchesShowingControllerKey
        ) as? ShowTouchesController
        if controller == nil {
    ...
    @objc(showTouches_sendEvent:)
    func showTouches_sendEvent(_ event: UIEvent) {
    ...
    @objc
    public static func startShowingTouches() {
        ...
        guard let originalMethod = class_getInstanceMethod(UIWindow.self, #selector(sendEvent(_:))),
              let newMethod = class_getInstanceMethod(
                  UIWindow.self,
                  #selector(showTouches_sendEvent(_:))
              )
        else {
            return
        }
